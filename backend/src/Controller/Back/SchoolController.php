...
{
    /**
     * @Route("/", name="app_back_school_index", methods={"GET"})
     */
    public function index(SchoolRepository $schoolRepository, Security $security): Response
    {     
        $user = $security->getUser();
        if ($this->isGranted('ROLE_ADMIN')) {
            $schoolsByUser = $schoolRepository->findAll();  
        } else {
            $schoolsByUser = $schoolRepository->findByMember($user);
        }
        
        return $this->render('back/school/index.html.twig', [
            'schools' => $schoolsByUser,
        ]);
    }

    /**
     * @Route("/new", name="app_back_school_new", methods={"GET", "POST"})
     */
    public function new(Request $request, SchoolRepository $schoolRepository, MySlugger $slugger, ManagerRegistry $doctrine, UserRepository $userRepository): Response
    {
        $school = new School();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($form->get('image')->getData()) {
                ...
                $school->setImage('data:'. $format . ';base64,' . $dataBase64);
            }
            ...
            if (!$school->getImage()) {
                $school->setImage($imageDefault);
            }
            if ($form->get('agendaLink')->getData()) {
                $agendaLinkUrl = $form->get('agendaLink')->getData();
                $agendaReplace = str_replace('<iframe src="', '', $agendaLinkUrl);
                $agendaLink = preg_replace('/ style="border.*$/','',$agendaReplace);
                $school->setAgendaLink($agendaLink);
            }
            $school->setSlug($slugger->slugify($school->getName()));
            $schoolRepository->add($school, true);

            return $this->redirectToRoute('app_back_school_index', [], Response::HTTP_SEE_OTHER);
        }

        return $this->renderForm('back/school/new.html.twig', [
            'school' => $school,
            'form' => $form,
        ]);
    }
    ...
    /**
     * @Route("/{id}/edit", name="app_back_school_edit", methods={"GET", "POST"})
     */
    public function edit(Request $request, School $school, SchoolRepository $schoolRepository): Response
    {
        ...
        return $this->renderForm('back/school/edit.html.twig', [
            'school' => $school,
            'form' => $form,
            'locations' => $locations,
        ]);
    ...
    /**
     * @Route("/switch/{id}", name="school_switch", methods={"GET","POST"}, requirements={"id"="\d+"})
     */
    public function switchSchool(School $school, Request $request, ManagerRegistry $doctrine): Response
    {
        $this->denyAccessUnlessGranted('SCHOOL_EDIT', $school);
        if ($school->isActivated()) {
            $school->setActivated(0);
        } else {
    ...
    /**
     * @Route("/accepted/{id}", name="school_accepted", methods={"GET","POST"}, requirements={"id"="\d+"})
     */
    public function acceptedSchool(School $school, Request $request, ManagerRegistry $doctrine): Response
    {
        $this->denyAccessUnlessGranted('SCHOOL_EDIT', $school);
        $school->setNewRequest(false);
    ...
    /**
     * @Route("/delete/{id}", name="app_back_school_delete", methods={"POST"})
     */
    public function delete(Request $request, School $school, SchoolRepository $schoolRepository): Response
    {
        ...
        $this->addFlash('success', 'L\'école a été refusée et donc supprimée de la base de donnée.');
